...
<html>
  ...
  ?>
    <div class="card">
      <div class="card-body">
        <h1 style="padding: 0 240px;">Add to the collection:</h1>
        <hr/>
          <?php
            // Initialization
            ...
            $forms = ['Natural/Raw', 'Tumbled', 'Polished', 'Shaped', 'Cluster', 'Point', 'Fossil', 'Geode', 'Volcanic',  ];
            
            if (isset($_POST['add_mineral_submission'], $_POST['mineral_name'],
                    ...
                    $_POST['comments']))
            {
                ...
                $checked_mineral_forms = $POST['mineral_form_checkbox'];
                
                $mineral_name = filter_var($mineral_name, FILTER_SANITIZE_STRING);
                $color_description = filter_var($color_description, FILTER_SANITIZE_STRING);
                ...
                */
                ...
                if (empty($file_error_message))
                {
                    ...
                    $results = parameterizedQuery($dbc, $query, 'ssisss', $mineral_name, $color_description, $weight_in_grams,
                            $mineral_form_text, $comments, $mineral_image_file_path);
                  ...
                  <div class="row">
                    <div class="col-2">
                      <img src="<?= $mineral_image_file_path ?>" class="img-thumbnail" style="max-height: 500px;" alt="Mineral image">
                    </div>
                    <div class="col">
            ...
            {
              ?>
        <form enctype="multipart/form-data" style="padding: 40px 240px;" class="needs-validation" novalidate method="POST" action="<?= $_SERVER['PHP_SELF']; ?>">
          <div class="form-group row">
            <label for="mineral_name" class="col-sm-3 col-form-label-lg">Name:</label>
            <div class="col-sm-8">
              <input type="text" class="form-control" id="mineral_name" name="mineral_name" value='<?= $mineral_name ?>' placeholder="Mineral/Fossil Name" required>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-8">
            <input type="text" class="form-control" id="color_description" name="color_description" value='<?= $color_description ?>' placeholder="Color Description" required>
              <div class="invalid-feedback">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="number" step="0.01" class="form-control" id="weight_in_grams" name="weight_in_grams" value='<?= $weight_in_grams ?>' placeholder="Weight (in grams)" required>
              <div class="invalid-feedback">
                ...
                foreach ($forms as $mineral_form)
                {
                  ...
                  <div class="form-check form-check-inline col-sm-3">
                    <input class="form-check-input" type="checkbox"
                        id="mineral_form_checkbox_action_<?= $mineral_form ?>"
                        name="mineral_form_checkbox[]"
                        value="<?= $mineral_form ?>">
                    <label class="form-check-label"
                        for="mineral_form_checkbox_action_<?= $mineral_form ?>"><?= $mineral_form ?></label>
                  </div>
                  <?php
                }
              ?>
          ...
          <button class="btn btn-primary" style="background-color: #5432a8;" type="submit" name="add_mineral_submission">Add Mineral/Fossil</button>
        </form>
        <script>
