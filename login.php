          ...
          <?php
            if (empty($_SESSION['user_id']) && isset($_POST['login_submission']))
            {
                ...
                if (!empty($user_name) && !empty($password))
                {
                    ...
                    $query = "SELECT id, user_name, password_hash, access_privileges FROM user WHERE user_name = ?";
        ...
        <form class="needs-validation" novalidate method="POST" action="<?= $_SERVER['PHP_SELF']; ?>">
          ...
          <?php
            elseif (isset($_SESSION['user_name'])):
                echo "<h4><p class='text-success'>You are logged in as: <strong>{$_SESSION['user_name']}</strong>.</p></h4>";
            endif;
          ?>
      </div>
    </div>
